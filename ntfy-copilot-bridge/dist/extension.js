...
function parseTargetControlCommand(message) {
...
function parseMessageTarget(message) {
    ...
    return {
        target: normalizeChatTarget(match[1]),
        text: match[2].trim()
    };
}
// ============================================================
...
class NtfyInputBridge {
    ...
    get defaultTarget() {
        return normalizeChatTarget(this.configuration.get('defaultTarget', 'auto'));
    }
    get copilotMention() {
        return this.configuration.get('copilotMention', '@copilot').trim();
    }
    get codexMention() {
        return this.configuration.get('codexMention', '@codex').trim();
    }
    get effectiveTarget() {
    ...
    startListening() {
        ...
        this.log.appendLine('[Input] 开始监听: ' + this.inputTopic +
    ...
    connect() {
        ...
        const req = proto.get(url, (res) => {
            ...
            res.on('error', (err) => this.handleDisconnected('[Input][流错误] ' + err.message));
            res.on('close', () => {
        ...
        req.on('error', (err) => this.handleDisconnected('[Input][连接错误] ' + err.message));
        this.request = req;
    }
    disconnect() {
    ...
    scheduleReconnect() {
        if (!this.wantListening || this.reconnectTimer)
            return;
    ...
    handleJsonLine(line) {
        ...
        if (controlTarget) {
            ...
            this.updateStatusBar();
            this.log.appendLine('[Input][路由] 默认目标已切换为: ' + describeChatTarget(controlTarget));
            void vscode.window.showInformationMessage('Ntfy Bridge: 默认目标切换为 ' + describeChatTarget(controlTarget));
            return;
        }
        const now = Date.now();
    ...
    applyTargetPrefix(message, target) {
        const trimmed = message.trim();
        ...
        const mention = target === 'copilot'
            ...
            : this.codexMention;
        if (!mention) {
            this.log.appendLine('[Input][路由] ' + describeChatTarget(target) +
                ' 未配置前缀，回退到原始消息');
            return trimmed;
        }
    ...
    async injectToChat(plan) {
        try {
            await vscode.commands.executeCommand('workbench.action.chat.open', { query: plan.injected, isPartialQuery: false });
            this.log.appendLine('[Input] 已注入聊天 | 目标=' + describeChatTarget(plan.target));
        }
        catch (err) {
            this.log.appendLine('[Input][注入失败] ' + (err?.message || err));
            try {
                await vscode.commands.executeCommand('workbench.action.chat.open', { query: plan.injected, isPartialQuery: true });
                await vscode.commands.executeCommand('workbench.action.chat.acceptInput');
                this.log.appendLine('[Input] 备用注入成功 | 目标=' + describeChatTarget(plan.target));
            }
            catch (err2) {
...
class ResponseWatcher {
    ...
    async processResponse() {
        try {
            ...
            if (!data.timestamp || data.timestamp <= this.lastProcessedTimestamp)
                return;
            this.lastProcessedTimestamp = data.timestamp;
            if (!data.images || data.images.length === 0)
                return;
            const config = vscode.workspace.getConfiguration('ntfyBridge');
            if (config.get('forwardImages') === false)
                return;
            const serverUrl = normalizeServerUrl(config.get('serverUrl'));
            const outputTopic = config.get('outputTopic', '').trim();
            if (!outputTopic)
                return;
    ...
    uploadImage(serverUrl, topic, imagePath) {
...
class ViewerServer {
    ...
    serveImage(res, filePath) {
        ...
        try {
            ...
            const allowedPaths = (responseData.images || []).map((img) => path.normalize(img.absPath));
...
function activate(context) {
    ...
    // 根据配置启动 Web 查看器
    const config = vscode.workspace.getConfiguration('ntfyBridge');
    if (config.get('viewerEnabled', false)) {
        viewer = new ViewerServer(log);
        viewer.start(config.get('viewerPort', 19199), config.get('viewerHost', '0.0.0.0'));
    }
    context.subscriptions.push(vscode.commands.registerCommand('ntfy-bridge.toggle', () => bridge?.toggle()), vscode.commands.registerCommand('ntfy-bridge.start', () => {
        ...
        const port = config.get('viewerPort', 19199);
