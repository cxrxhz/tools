...
# 正则表达式模式匹配温度值（支持小数）
temperature_pattern = re.compile(r"(\d+\.?\d*)K")
...
for subdir in os.listdir(root_directory):
    ...
    if os.path.isdir(subdir_path):
        ...
        # Initialize lists to store results for this dataset
        temperatures = []
        dR1_values = []
        dR2_values = []
        ...
        for file_name in os.listdir(subdir_path):
            if file_name.endswith('.txt'):
                # 提取文件名中的温度信息
                match = temperature_pattern.search(file_name)
                if match:
                    temperature = float(match.group(1))
                else:
                    print(f'文件 {file_name} 不包含有效的温度信息，已跳过。')
                    continue
