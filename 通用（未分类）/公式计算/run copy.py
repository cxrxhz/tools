...
from sympy import symbols, latex, N, acos, Rational
...
class FormulaCalculatorApp:
    ...
    def create_assignment_window(self, expr, lhs, local_symbols):
        ...
        tk.Label(self.assign_window, text="请为以下变量输入数值或公式（如 2 或 a+1）：").pack(pady=5)
        self.assign_entries = {}
        self.entries_order = []
        vars_needed = [str(v) for v in expr.free_symbols if str(v) not in self.resolved_values]
        self.vars_needed = vars_needed
        ...
        tk.Button(self.assign_window, text="下一步", command=lambda: self.process_assignments_v3(expr, lhs)).pack(pady=10)
    ...
    def process_assignments_v3(self, expr, lhs):
        assignments = {}
        formula_inputs = {}
        # 先收集所有输入
        ...
        def resolve_var(var, resolving_stack=None):
            ...
            if var in formula_inputs:
                ...
                # 解析公式
                try:
                    vars_in_expr = set(re.findall(r'\b[A-Za-z_][A-Za-z0-9_]*\b', formula_inputs[var]))
                    math_funcs = {'sin', 'cos', 'tan', 'tanh', 'sqrt', 'log', 'exp', 'ln',
                                  'sinh', 'cosh', 'asin', 'acos', 'atan', 'atanh', 'pi', 'E'}
                    vars_in_expr -= math_funcs
                    sub_symbols = symbols(list(vars_in_expr))
                    sub_symbols_dict = {str(s): s for s in sub_symbols}
                    sub_expr = parse_expr(formula_inputs[var], local_dict=sub_symbols_dict)
                    sub_assignments = {}
                    for sub_var in vars_in_expr:
                        if sub_var in self.resolved_values:
                            sub_assignments[sub_var] = self.resolved_values[sub_var]
                        else:
                            # 递归弹窗输入
                            sub_val = self.prompt_subformula_v2(sub_var)
                            if sub_val is None:
                                return None
                            sub_assignments[sub_var] = sub_val
                            self.resolved_values[sub_var] = sub_val
                    final_val = float(N(sub_expr.subs(sub_assignments), 15))
                    self.resolved_values[var] = final_val
                    self.process_log.append({
                        "variable": var,
                        "formula": f"{var} = {formula_inputs[var]}",
        ...
        main_assignments = {}
        for var in self.vars_needed:
            val = resolve_var(var)
            if val is None:
                return
            main_assignments[var] = val
        ...
        final_expr = expr.subs(main_assignments)
        try:
    ...
    def prompt_subformula_v2(self, var_name):
        result_holder = {}
        sub_window = tk.Toplevel()
        sub_window.title(f"请输入变量 {var_name} 的数值或公式")
        self.center_window(sub_window, 600, 200)
        sub_window.grab_set()
        tk.Label(sub_window, text=f"请输入 {var_name} 的数值或公式（如 2 或 a+1）：").pack(pady=10)
        entry = tk.Entry(sub_window, width=70)
        entry.pack()

        def submit_sub():
            val = entry.get().strip().replace("，", ".")
            if val == "":
                messagebox.showerror("错误", f"变量 {var_name} 未赋值，请输入数值或公式。", parent=sub_window)
                return
            try:
                num_val = float(val)
                result_holder[var_name] = num_val
                sub_window.destroy()
            except ValueError:
                # 公式，递归处理
                try:
                    vars_in_expr = set(re.findall(r'\b[A-Za-z_][A-Za-z0-9_]*\b', val))
                    math_funcs = {'sin', 'cos', 'tan', 'tanh', 'sqrt', 'log', 'exp', 'ln',
                                  'sinh', 'cosh', 'asin', 'acos', 'atan', 'atanh', 'pi', 'E'}
                    vars_in_expr -= math_funcs
                    sub_symbols = symbols(list(vars_in_expr))
                    sub_symbols_dict = {str(s): s for s in sub_symbols}
                    sub_expr = parse_expr(val, local_dict=sub_symbols_dict)
                    sub_assignments = {}
                    for sub_var in vars_in_expr:
                        if sub_var in self.resolved_values:
                            sub_assignments[sub_var] = self.resolved_values[sub_var]
                    ...
                    self.process_log.append({
                        "variable": var_name,
                        "formula": f"{var_name} = {val}",
                        "assignments": sub_assignments,
                        "expr": sub_expr
                    })
                    result_holder[var_name] = final_val
                    sub_window.destroy()
                except Exception as e:
                    messagebox.showerror("错误", f"变量 {var_name} 的公式解析失败：{e}", parent=sub_window)
        tk.Button(sub_window, text="提交", command=submit_sub).pack(pady=10)
