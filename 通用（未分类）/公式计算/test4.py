...
class FormulaCalculatorApp:
    ...
    def show_final_result(self, lhs, final_value):
        ...
        # 添加窗口关闭协议
        final_window.protocol("WM_DELETE_WINDOW", self.quit_app)  # 新增
    ...
    def quit_app(self):
        self.master.quit()  # 终止主循环
        self.master.destroy()  # 销毁所有残余窗口
...
if __name__ == "__main__":
    ...
    root.withdraw()
    top = tk.Toplevel()
    # 当关闭 top 窗口时，同时销毁 top 和隐藏的 root 窗口，从而退出程序
    top.protocol("WM_DELETE_WINDOW", lambda: (top.destroy(), root.destroy()))
    app = FormulaCalculatorApp(top)
    root.mainloop()
